...
from preprocess import * #  gen_data_set, get_normalization_layer, get_category_encoding_layer, plot_loss, p2rDays
...
str_cat_feas = ['user_key', 'device_model', 'device_brand', 'channel', 'app_version', 'platform','net_type', 'user_channel']  # , 'login' bool  #
...
def build_model():
    embedding_dim = 8  # 类别特征emb维度
    select_userfeas = ['user_key', 'device_model', 'device_brand']  # 选择除了hist_len和hist_item_id_pad以外的特征
    select_itemfeas = ['item_id', 'collect_cnt']
    select_feas = select_userfeas + select_itemfeas
    user_features, item_features = prepareFeas(embedding_dim, seqL, select_feas, user_feas, item_feas, 
                                                fea_max_idx, float_num_feas, int_num_feas,int_cat_feas, str_cat_feas,False)
    
    model_dssm = DSSM(user_features, item_features,  # , model_processed,
                      user_dnn_hidden_units=(64, 32),  # 64,16
                      item_dnn_hidden_units=(32, 32),  # 32,16
                      dnn_activation='tanh',
                      dnn_use_bn=False,
                      l2_reg_dnn=0,
    ...
    model = tf.keras.Model(inputs, model_dssm(processed))
    model.compile(
                    optimizer='rmsprop',  # ,  # 'rmsprop''adagrad',
                    loss="binary_crossentropy",
                    metrics=[AUC(name='AUC'), BinaryAccuracy(name='Accuracy'), Precision(name='Precision'), Recall(name='Recall'),]
                  )  #
...
# models can plot ::: model_processed, model_dssm, model, model.layers[-1], user_model, user_model.layers[-1]
...
user_model.save(Path(model_export_path+'u_emb/'+model_version), save_format="tf")  # !!!, save_traces=False 会减少序列化文件大小和时间，但是需要自定义层定义config方法
item_model.save(Path(model_export_path+'i_emb/'+model_version), save_format="tf")  # !!!, save_traces=False 
# %% test model
...
item_emb_json = [{"item_id": str(i), "gender": j.tolist(), "vector": k.tolist()}
                  for i, j, k in zip(test_item_input['item_id'], test_item_input['gender'], test_item_input['vector'])]
